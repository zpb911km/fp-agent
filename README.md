...
**FP** 是以 LLM 为推理核心、工具为执行手臂的自主 Agent 引擎。它不是简单的 Chatbot 封装，而是一个可编程的 **思考-行动循环** 框架。
...
> **本项目 100% 的代码和文件，均由「FP」(这个 AI Agent 系统) 自己编写。**
...
| 🔌 **生命周期插件** | 15 个生命周期钩子（8 transform + 7 observe）覆盖全流程——"文件即开关"，改文件名即改配置 |
| 🛠️ **工具系统** | 内置 14 个工具（4 核心 + 10 插件；bash、文件读写、网络抓取、记忆、任务管理等），支持用户目录扩展 |
...
│   ├── commands/                # 15 个命令（14 内置文件命令 + 1 插件注入 `/sc`；其中 1 个 `!` 特殊命令）
│   ├── plugins/                 # 生命周期插件系统
│   ├── tools/                   # 工具注册与执行
│   └── prompts/                 # 系统提示词
