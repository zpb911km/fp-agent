...
class SkillLoader:
    ...
    def load_all(self) -> dict[str, Skill]:
        """加载所有技能（内置 → 用户，同名覆盖）"""
        self.skills.clear()
        self._load_from_dir(BUILTIN_SKILLS_DIR)
    ...
    def _load_from_dir(self, directory: str) -> None:
        ...
        for filename in os.listdir(directory):
            ...
            skill = self._load_skill_file(os.path.join(directory, filename))
            if skill:
                self.skills[skill.name] = skill

    def _load_skill_file(self, filepath: str) -> Skill | None:
        """加载单个技能文件"""
        try:
    ...
    def _parse_skill(self, content: str, filepath: str = "") -> Skill | None:
        ...
        if match:
            ...
            return Skill(name=name, title=name.replace("_", " ").title(), description=content[:200], content=content)
...
def create_skill_loader() -> SkillLoader:
    """创建独立的 SkillLoader 实例并加载所有技能。

    与全局 `skill_loader` 完全隔离，适用于多 Agent 实例场景。
    """
    loader = SkillLoader()
    loader.load_all()
    return loader
