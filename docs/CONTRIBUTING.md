...
感谢你考虑为 FP Agent 贡献代码！这份指南帮助你快速了解如何参与项目。

> **在开始之前，请先阅读 [项目原则](PRINCIPLES.md) —— 它告诉你这个项目是怎么被创造出来的。**
> 简而言之：代码由 FP 在与人类的对话中编写。如果你有想法，先对话，再改代码。
...
- **提交时（提醒+阻断）**：pre-commit 钩子 `docs-sync` 检查本次 git 变更：
  如果**代码变了、而相关文档没同步变**，输出受影响文档清单并**拒绝提交**。
  判定基于 git 变更流（代码路径 → 关联文档的映射规则在
  `scripts/check_docs_sync.py`），不依赖文本比对——新增/改名都能发现。
  **按变更类型分层**：内容修改（改现有文件）只要求 dev 行为文档确认；
...
- **规则审计**：`python scripts/check_docs_sync.py --audit`
  ...
  同时校验 `DOC_LINKS` 反向关联：文档登记的「源码真相」代码文件若已不存在
  （被重构/改名），会报链接断裂。

代码变更文件与受影响文档的对应规则集中在 `scripts/check_docs_sync.py`
的 `DOC_RULES`（命令/工具/钩子/核心/界面层/脚本各自关联到对应文档）。
反向的 `DOC_LINKS`（文档 → 关联代码文件）用于：
文档变更时提醒关联代码未同步变更（**不阻断**，文档允许先行编写/补充说明），
以及 `--audit` 时校验文档里的代码引用是否仍存在。
